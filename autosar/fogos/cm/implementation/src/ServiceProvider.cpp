...
: m_serviceId(serviceId), m_instanceId(instanceId), m_mode(mode), m_clientId(0), m_session(0), m_mutex(), m_condition(), m_terminated(false)
{
	if (m_mode == MethodCallProcessingMode::kEventSingleThread)
	{
		std::thread singleThread([this](){
