...
bool Deserializer::deserialize(std::string& value, std::string encoding)
{
	uint8_t val = 0;
	uint32_t len = 0;
	...
	if (encoding == "UTF-8")
	{
		if (len < 4 || m_size < len + 1) //BOM+terminate
		{
			return false;
		}
		
		if (0xEF == *m_pos++ && 0xBB == *m_pos++ && 0xBF == *m_pos++)
		{
