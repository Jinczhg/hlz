...
#define DEFAULT_BUFSIZE 51200 //50K
...
check_bpf_dev:
    if (access(BPF_NODE, F_OK) != 0)
    {
        try_count++;
	if (try_count % 100 == 0)
	{
	    ERROR("%s: bpf device not exist", BPF_NODE);
	}
